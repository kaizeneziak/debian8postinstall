...
RETOUR=$(exec_command "sed -i 's/$VAR_SQL_MARIADB_PORT/$PORT_SQL/g' $FILE_MARIADB_MY_CNF")
if [ $RETOUR -eq 0 ]; then
	aff_message "ok" "Remplacement du port d'écoute du serveur MariaDB $(aff_important "$VAR_SQL_MARIADB_PORT") par $(aff_important "$PORT_SQL")"
else
	aff_message "ok" "Remplacement du port d'écoute du serveur MariaDB $(aff_important "$VAR_SQL_MARIADB_PORT") p
ar $(aff_important "$PORT_SQL")"
fi
...
if [ $? -eq 0 ]; then
	aff_message "ok" "Modification du nom d'utilisateur root en $(aff_important "$COMPTE_ROOT")"
else
	aff_message "ok" "Modification du nom d'utilisateur root en $(aff_important "$COMPTE_ROOT")"
fi
exec_service_restart "$VAR_SERVICE_MARIADB"
